...
const { fetchUserById, patchUserById } = require('../user-utils')
...
setupApiUrl(`${apiUrl}/{users.user1.id}`)
...
test('(200) user updated', async t => {
    ...
    t.context.users.user1 = await patchUserById(t.context.users.user1.id, {
    ...
    const userFromDb = await fetchUserById(t.context.users.user1.id)
